...
const String HTML_CSS_STYLING = "<head><meta charset='UTF-8'><title>EvilCrowRF</title><link href='style.css' rel='stylesheet'><meta name='viewport' content='width=device-width, initial-scale=1.0'><script src='lib.js'></script><link rel='icon' href='/ECRF.ico' type='image/x-icon'></head>";
...
bool checkReceived(void) {
  delay(1);  // Este atraso pode não ser necessário e pode afetar o desempenho em comunicações de alta velocidade.
  if (samplecount >= minsample && micros() - lastTime > 100000) {
    detachInterrupt(RXPin0);
    detachInterrupt(RXPin);
    return true;  // Use "true" em vez de "1" para retorno booleano.
  } else {
    return false;  // Use "false" em vez de "0" para retorno booleano.
  }
}
...
void printReceived() {
  ...
  for (int i = 0; i < samplecount; i++) {  // Corrija o índice inicial.
...
void RECEIVE_ATTR receiver() {
  ...
  if (samplecount >= samplesize) {
    // Verifique se o retorno de checkReceived() é tratado aqui de acordo com sua lógica.
    bool received = checkReceived();
    // Faça algo com a variável "received" se necessário.
    if (received) {
      // Realize ações quando a comunicação for recebida com sucesso.
    }
...
void parse_data() {
  ...
  for (int d = 0; d<data_count; d++){
    ...
    String rawBinary = bindataprotocol.substring(data_begin_bits + 15, data_end_bits - 2);
    rawBinary.replace(" ", "");
...
void setup() {
  ...
  controlserver.on("/logger", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(SD, "/HTML/logger.html", "text/html");
  });
  controlserver.on("/rxconfig", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(SD, "/HTML/rxconfig.html", "text/html");
  });

    controlserver.on("/bruteforce", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(SD, "/HTML/bruteforce.html", "text/html");
  });
  ...
  controlserver.on("/txprotocol", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    controlserver.on("/rfanalize", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(SD, "/HTML/rfanalyze.html", "text/html");
  });
  ...
  controlserver.on("/updatesd", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    controlserver.on("/ECRF.ico", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(SD, "/HTML/ECRF.ico", "text/html");
  });
...
controlserver.on("/listxmlfiles", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    if (dir) {
        while (File file = dir.openNextFile()) {
            ...
            fileList += String(file.name()) + "\n"; // Adicione o nome do arquivo com uma quebra de linha
            file.close();
        }
        
        dir.close();
    } else {
        fileList = "Erro ao abrir o diretório.";
    }
...
controlserver.on("/setbruteforce", [](AsyncWebServerRequest *request){
    tmp_module = request->arg("module");
    tmp_frequency = request->arg("frequency");
    tmp_code = request->arg("code");
    tmp_codelen = request->arg("bitlenght");
    if (request->hasArg("bruteforce")) {
      frequency = tmp_frequency.toFloat();
      code = strtol(tmp_code.c_str(), NULL, 0);
      codelen = tmp_codelen.toInt();

      // Configuração do pino de saída com base em tmp_module
      int outputPin;
      if (tmp_module == "1") {
        outputPin = 2;
      } else if (tmp_module == "2") {
      ...
      // Configuração do módulo CC1101 com base em tmp_module
      if (outputPin != 0) {
        ...
        for (int y = 0; y < 99999999999; y++) {
          // Envia bits individualmente com base em code e codelen
          for (int8_t bit = codelen - 1; bit >= 0; bit--) {
            digitalWrite(outputPin, (code & (1 << bit)) != 0 ? HIGH : LOW);
            delayMicroseconds(pulseWidth);
          }

          code++; // Avança para o próximo valor de code
          delay(100); // Atraso entre envios
        }

        digitalWrite(outputPin, LOW); // Certifique-se de que o pino esteja em LOW após o loop
      }
    }
});
...
controlserver.on("/eraseBin", HTTP_GET, [](AsyncWebServerRequest *request) {
    File rawDataSD = SD.open("/rawdata.txt", "w");
  if (rawDataSD) {  
      // Agora você pode escrever novos dados no arquivo
      rawDataSD.println("");     
...
controlserver.on("/getRawData", HTTP_GET, [](AsyncWebServerRequest *request){
    File file = SD.open("/rawdata.txt", "r"); // Abra o arquivo logs.txt no modo de leitura
  ...
  controlserver.on("/lib.js", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(SD, "/HTML/javascript.js", "text/javascript");
     ...
     controlserver.on("/cdn.jsdelivr.net_npm_chart.js", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(SD, "/HTML/cdn.jsdelivr.net_npm_chart.js", "text/javascript");
  });
   ...
   controlserver.on("/bootstrap.bundle.min.js", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(SD, "/HTML/bootstrap.bundle.min.js", "text/javascript");
  });

   controlserver.on("/notify.min.js", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(SD, "/HTML/notify.min.js", "text/javascript");
  });
  ...
  if (firstsample == sample[1] and firstsample < timingdelay[0]){
    ...
    rawDataSD = SD.open("/rawdata.txt", "w");
  if (rawDataSD) {  
      // Agora você pode escrever novos dados no arquivo
      rawDataSD.println("");     
      // Fecha o arquivo quando terminar
    ...
    if (calculate>0){
      ...
      //Serial.println(" samples]");
        ...
        for (int b=0; b<calculate; b++){
         // Serial.println(lastbin);
           rawDataSD.print(lastbin);
